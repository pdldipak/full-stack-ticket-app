...
Ticket codes use **`TKT-{slug}-{id}`** (e.g. `TKT-NY-2083-0004`) by default. The default slug lives in **`backend/src/config/ticketEventConfig.js`** and should match **`frontend/src/config/eventConfig.js`** (`TICKET_CODE_EVENT_SLUG`). Optional env **`TICKET_CODE_EVENT_SLUG`** overrides that default; **`TICKET_CODE_LEGACY=1`** forces old **`TKT-{id}`**-only codes (no slug). The API loads **`backend/.env`** for local dev; **variables already set in the environment (e.g. Docker Compose) are not overwritten** by that file. QR codes encode that ticket code as PNG **base64 data URLs** stored in `qr_image_base64`. The UI treats **price** as **Swedish kronor (SEK)**; the database stores a numeric amount only. Each ticket has a **`city`** (program location: Stockholm or Gothenburg). **Payment** fields: **`paid`** (yes/no) and **`paid_to`** (money to the **seller** vs **NRNA NCC account**).
